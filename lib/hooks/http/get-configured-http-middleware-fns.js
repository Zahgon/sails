 ...
 *
 * @param  {Fastify} app [the Fastify app instance]
 ...
 *             @param {Function} next
 */
module.exports = function getBuiltInHttpMiddleware (app, sails) {
  ...
  return _.defaults(sails.config.http.middleware || {}, {
    ...
    // If a Connect session store is configured, hook it up to the HTTP middleware pipeline
    session: (function() {
    ...
    bodyParser: (function() {
      ...
      opts.onBodyParserError = function (err, req, res, next) {// eslint-disable-line no-unused-vars
        ...
        // to have special meaning as an error handler (i.e. to the HTTP middleware pipeline)
