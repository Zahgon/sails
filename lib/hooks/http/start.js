...
module.exports = function (sails) {
  ...
  return function startServer (cb) {
    ...
    async.auto({
      ...
      // Finish booting the Fastify app, then start its underlying HTTP server
      start: function (next) {
        ...
        sails.hooks.http.app.ready(function (err) {
          if (err) { return next(err); }

          // If host is explicitly declared, include it in the listen() call
          if (explicitHost) {
    ...
    }, function httpListening (err) {
      clearTimeout(liftAbortTimer);
      if (err) { return cb(err); }

      // Announce that the HTTP server is now listening on a port
